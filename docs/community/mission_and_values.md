...
- providing a **consistent and stable API** to enable plugin developers to build on top of napari without their code constantly breaking and to enable advanced users to build out sophisticated Python workflows. We follow a formal [deprecation policy](napari-deprecation-policy), which commits us to warning you in code and to publishing the window in which deprecated API may be removed.
