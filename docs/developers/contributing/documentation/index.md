...
- a clean `conda` environment—remember to activate it!;
- a from-source, editable installation of napari with the optional `docs` dependencies and a Qt backend. Change directories to where you cloned napari and run, for example:

  ```bash
  python -m pip install -e ".[pyqt, docs]"
  ```
  This will use the default Qt backend. For other options, see [the napari installation guide](https://napari.org/stable/tutorials/fundamentals/installation.html#choosing-a-different-qt-backend).

  ````{note}
  You can combine the documentation dependencies with [a development installation of napari](dev-installation) by adding `docs` to the dependencies in `[]`, e.g. `[pyqt, dev, docs]`.
  ````

This setup, with these dependencies, will allow you to preview your document locally as it would appear on `napari.org`.  
